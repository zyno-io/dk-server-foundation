...
Configure via `MUTEX_MODE` (`local` or `redis`) and `MUTEX_REDIS_*` environment variables.
...
```typescript
import { createBroadcastChannel, createDistributedMethod } from '@signal24/dk-server-foundation';
...
class CacheManager {
    constructor() {
        createDistributedMethod(this, 'invalidate', async (data: { key: string }) => {
...
Uses `BROADCAST_REDIS_*` environment variables.
